...
export default function AdminSidebar({ isOpen, toggleSidebar }) {
  ...
  const menuItems = [
    ...
    { id: '/admin/contactmaster', label: 'Contact Master', icon: <FileSpreadsheet size={20} /> },
    { id: '/admin/infomaster', label: 'Info Master', icon: <FileSpreadsheet size={20} /> },
  ];
    ...
    >
      ...
      <nav className="flex flex-col space-y-2 flex-1">
        {menuItems.map((item) => (
          ...
          >
            <div className={`p-2 rounded-xl transition-all duration-300 group-hover:scale-110 flex-shrink-0 ${
              active === item.id
            ...
            <span className="text-lg font-semibold tracking-wide flex-1">{item.label}</span>
